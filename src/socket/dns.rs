...
impl<'a> DnsSocket<'a> {
    ...
    pub fn start_query(&mut self, name: &[u8]) -> Result<QueryHandle> {
    ...
    pub(crate) fn process(
        ...
        if !p.flags().contains(Flags::RESPONSE) {
            net_trace!("{}: packet doesn't have response bit set", self.meta.handle);
            return Err(Error::Malformed);
        }
        ...
        for q in self.queries.iter_mut().flatten() {
            if let State::Pending(pq) = &mut q.state {
                ...
                if p.rcode() == Rcode::NXDomain {
                    net_trace!("{}: rcode NXDomain", self.meta.handle);
                ...
                for _ in 0..p.answer_record_count() {
                    ...
                    match r.data {
                        ...
                        RecordData::Cname(name) => {
                            ...
                            pq.txid = rand::rand_u16();
                            pq.port = rand::rand_source_port();
                        }
        ...
        for q in self.queries.iter_mut().flatten() {
            if let State::Pending(pq) = &mut q.state {
                ...
                let ip_repr = IpRepr::Unspecified {
                    ...
                    dst_addr: self.servers[pq.server_idx],
                ...
                if let Err(e) = emit((ip_repr, udp_repr, payload)) {
